...
<app:layout mode="header" />
	...
	<form:form commandName="model" id="model" method="post">
		...
		<input type="hidden" name="bbsType" id="bbsType" value="${paramMap.bbsType }" />
		
		<div class="titWrap">
			<div class="subject">
				<form:select path="searchCond">
					<form:option value="subjectContent">전체</form:option>
					<form:option value="subject">제목</form:option>
					<form:option value="content">답변</form:option>
				</form:select>
			</div>
		...
		<div class="list_faq">
			...
			<ul class="infoWrap">
				<li>
					<ul>
						<li><input type="checkbox" name="ckall" id="chk"></li>
						<li>번호</li>
						<li>FAQ 제목</li>
						<li>작성자</li>
						<li>작성일</li>
				...
				<c:forEach items="${pageList }" var="item" varStatus="status">
					<li>
						<ul>
							<li><input type="checkbox" name="ck" id="chk" value="${item.bbsNo }"></li>
							<li>${item.rNum }</li>
							<li><a href="javascript:viewFaq(${item.bbsNo })">${item.bbsSubject }</a></li>
<%-- 							<li><a href="#void" class="view">${item.bbsSubject }</a></li> --%>
							<li>${item.register }</li>
							<li>${item.rgsDe }</li>
							<li>${item.viewCnt }</li>
						</ul>
